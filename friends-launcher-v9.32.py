# friends-launcher-v9.32-win.py
#
# Based on friends-launcher-v9.31-win.py
# Changes from v9.31:
# - Added window icon (Friends logo) for the title bar on Windows.
# - Added attempt to enable dark title bar on Windows 10/11 using DWM API.
...
EXPECTED_DWEB_SHA256 = "d2225d9c3f61c4715742122776c697824d747bccb842e95efcd485f00cb67b64"
...
class DwebLauncherWin:
    def __init__(self, master):
        ...
        # Try to set window icon (Friends logo) on Windows
        if is_windows():
            try:
                self.master.iconbitmap(ICON_FILENAME)
            except Exception:
